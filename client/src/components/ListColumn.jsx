      ...
      <Draggable draggableId={list.id} index={index}>
        {(provided, snapshot) => (
          <div
            ...
            className={`rounded-xl w-72 flex-shrink-0 flex flex-col max-h-full border
              ${snapshot.isDragging ? 'shadow-xl rotate-1' : ''}`}
            style={{
              background: 'var(--bg-surface)',
              borderColor: snapshot.isDragging ? 'rgba(99, 102, 241, 0.4)' : 'var(--border)',
              boxShadow: snapshot.isDragging ? 'var(--glow-hover)' : 'none',
            ...
            >
              {editingTitle ? (
                <input
                  ...
                  className="flex-1 font-semibold text-sm rounded px-1.5 py-0.5 outline-none text-gray-200"
                  style={{ background: 'var(--bg-elevated)', border: '2px solid rgba(99, 102, 241, 0.5)' }}
                />
              ) : (
                <h3
                  onClick={() => setEditingTitle(true)}
                  className="flex-1 font-semibold text-sm text-gray-200 cursor-pointer hover:bg-white/5 rounded px-1.5 py-0.5 truncate"
                >
                  {list.title}
                </h3>
              )}
              ...
              <button
                ...
                className="ml-2 text-gray-500 hover:text-red-400 transition-colors p-1 rounded hover:bg-white/5"
              >
              ...
              {addingCard ? (
                <div>
                  <textarea
                    ...
                    rows={2}
                    className="w-full rounded-lg px-3 py-2 text-sm outline-none resize-none text-gray-200"
                ...
                <button
                  ...
                  className="w-full text-left text-sm text-gray-400 hover:text-gray-200 hover:bg-white/5 rounded-lg px-3 py-2 transition-colors flex items-center gap-1"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
