...
async function main() {
  ...
  await page.screenshot({
    path: `${SCREENSHOTS_DIR}/01-login.png`,
    fullPage: true,
  });
  ...
  await page.waitForSelector('form');
  await page.screenshot({
    path: `${SCREENSHOTS_DIR}/02-register.png`,
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: `${SCREENSHOTS_DIR}/03-boards-empty.png`,
    fullPage: true,
  });
  ...
  await page.waitForTimeout(500);
  await page.screenshot({
    path: `${SCREENSHOTS_DIR}/04-boards-with-data.png`,
    fullPage: true,
  });
  ...
  const cl = await (
    await fetch(`${API}/cards/${card1.data.id}/checklists`, {
      method: 'POST',
      headers,
  ...
  const item1 = await (
    await fetch(
      `${API}/cards/${card1.data.id}/checklists/${cl.data.id}/items`,
      {
        method: 'POST',
        headers,
  ...
  // Check one item
  await fetch(
    `${API}/cards/${card1.data.id}/checklists/${cl.data.id}/items/${item1.data.id}`,
    {
      method: 'PATCH',
      headers,
    }
  ...
  await fetch(`${API}/lists/${list2.data.id}/cards/${card2.data.id}`, {
    ...
    body: JSON.stringify({
      dueDate: '2026-03-10',
      description:
        'Implement JWT authentication with refresh tokens. Include login, register, and token refresh endpoints.',
    }),
  });
  ...
  await page.screenshot({
    path: `${SCREENSHOTS_DIR}/05-board-view.png`,
    fullPage: true,
  });
  ...
  await page.waitForTimeout(500);
  await page.screenshot({
    path: `${SCREENSHOTS_DIR}/06-card-modal.png`,
    fullPage: true,
  });
  ...
  await page.waitForTimeout(500);
  await page.screenshot({
    path: `${SCREENSHOTS_DIR}/07-card-modal-overdue.png`,
    fullPage: true,
  });
